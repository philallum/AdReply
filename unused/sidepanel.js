...
function setupEventListeners() {
    ...
    if (templateSearch) {
        templateSearch.addEventListener('input', debounce((e) => {
            ...
            filterTemplates(searchTerm, getCurrentFilter(), categoryFilter);
        }, 300));
    }
    ...
    filterButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            const categoryFilter = getCurrentCategoryFilter();
            filterTemplates(searchTerm, filterType, categoryFilter);
        });
    });
    ...
    if (templateCategoryFilter) {
        templateCategoryFilter.addEventListener('change', (e) => {
            const selectedCategory = e.target.value;
            handleTemplateCategoryFilter(selectedCategory);
        });
    }
...
// Placeholder for import/export handlers (will be implemented in later tasks)
function setupImportExportHandlers() {
    // This will be implemented when import/export functionality is added
    console.log('Import/Export handlers setup - placeholder');
}
...
function filterTemplates(searchTerm = '', filterType = 'all', categoryFilter = '') {
    ...
    if (categoryFilter) {
        filteredTemplates = filteredTemplates.filter(template => 
            ...
            (template.verticals && template.verticals.includes(categoryFilter))
        );
    }
...
async function loadUsageStatistics() {
    try {
        ...
        usageStatsContainer.innerHTML = `
            <div class="loading-stats p-6 text-center">
                <div class="spinner-medium mx-auto mb-3"></div>
                <p class="text-sm text-gray-600">Loading usage statistics...</p>
            </div>
        `;
        
        // Get usage data (this would normally come from storage/tracking)
        const usageData = await getUsageStatistics();
        
        if (!usageData || Object.keys(usageData).length === 0) {
            usageStatsContainer.innerHTML = `
                <div class="no-stats p-6 text-center text-facebook-gray">
                    <div class="mb-2">
        ...
        renderUsageStatistics(usageData);
        
    } catch (error) {
        console.error('Failed to load usage statistics:', error);
        const usageStatsContainer = document.getElementById('usageStatsContainer');
        usageStatsContainer.innerHTML = `
            <div class="error-stats p-6 text-center">
                <div class="mb-2">
                    <svg class="w-8 h-8 mx-auto text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                </div>
                <p class="text-sm text-red-600 mb-2">Failed to load usage statistics</p>
                <button onclick="loadUsageStatistics()" class="text-facebook-blue hover:text-facebook-hover text-sm font-medium">
                    Try Again
                </button>
            </div>
        `;
...
function renderUsageStatistics(usageData) {
    const usageStatsContainer = document.getElementById('usageStatsContainer');
    ...
    if (Object.keys(categoryStats).length > 0) {
        const categoryItems = Object.entries(categoryStats)
            ...
            .map(([categoryId, stats]) => {
                const percentage = totalUsage > 0 ? Math.round((stats.usage / totalUsage) * 100) : 0;
                return `
                    <div class="category-stat-item flex items-center justify-between p-2 bg-gray-50 rounded">
                        <div class="flex items-center space-x-2">
                            <span class="category-indicator bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full text-xs font-medium">
                                ${getCategoryDisplayName(categoryId)}
                            </span>
                            <span class="text-xs text-gray-600">${stats.templateCount} templates</span>
                        </div>
                        <div class="flex items-center space-x-2">
                            <div class="usage-bar bg-gray-200 rounded-full h-2 w-16">
                                <div class="bg-blue-500 h-2 rounded-full" style="width: ${percentage}%"></div>
                            </div>
                            <span class="text-xs font-medium text-gray-900">${stats.usage}</span>
                        </div>
                    </div>
                `;
            }).join('');
        ...
        categoryStatsHTML = `
            <div class="category-stats mb-6">
                <h3 class="text-sm font-medium text-gray-900 mb-3">Usage by Category</h3>
                <div class="category-breakdown space-y-2">
                    ${categoryItems}
                </div>
            </div>
        `;
    ...
    // Build top templates HTML
    let topTemplatesHTML = '';
    if (topTemplates.length > 0) {
        const templateItems = topTemplates.slice(0, 3).map((template, index) => `
            <div class="template-rank-item flex items-center justify-between p-2 bg-gray-50 rounded">
                <div class="flex items-center space-x-2">
                    <span class="rank-badge bg-yellow-100 text-yellow-800 w-6 h-6 rounded-full text-xs font-bold flex items-center justify-center">
                        ${index + 1}
                    </span>
                    <div>
                        <div class="text-xs font-medium text-gray-900 truncate max-w-32">
        ...
        topTemplatesHTML = `
            <div class="top-templates mb-6">
                <h3 class="text-sm font-medium text-gray-900 mb-3">Most Used Templates</h3>
                <div class="template-rankings space-y-2">
                    ${templateItems}
                </div>
            </div>
        `;
    ...
    if (recentActivity.length > 0) {
        const activityItems = recentActivity.slice(0, 3).map(activity => `
            <div class="activity-item flex items-center justify-between p-2 bg-gray-50 rounded text-xs">
                <div class="flex items-center space-x-2">
                    <div class="activity-icon w-4 h-4 bg-blue-100 rounded-full flex items-center justify-center">
                        <svg class="w-2 h-2 text-blue-600" fill="currentColor" viewBox="0 0 8 8">
                            <circle cx="4" cy="4" r="3"/>
                        </svg>
                    </div>
                    <span class="text-gray-900">${escapeHtml(activity.templateLabel)}</span>
                    ${activity.category ? `
                        <span class="text-gray-500">in ${getCategoryDisplayName(activity.category)}</span>
                    ` : ''}
                </div>
                <span class="text-gray-500">${formatTimeAgo(activity.timestamp)}</span>
            </div>
        `).join('');
    ...
    const statsHTML = `
        <div class="usage-stats-content p-4">
            <div class="stats-overview grid grid-cols-2 gap-4 mb-6">
                <div class="stat-card bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <div class="stat-value text-2xl font-bold text-blue-600">${totalUsage}</div>
                    <div class="stat-label text-xs text-blue-700">Total Uses</div>
                </div>
                <div class="stat-card bg-green-50 border border-green-200 rounded-lg p-3">
                    <div class="stat-value text-2xl font-bold text-green-600">${Object.keys(categoryStats).length}</div>
                    <div class="stat-label text-xs text-green-700">Active Categories</div>
                </div>
            </div>
            ${categoryStatsHTML}
            ${topTemplatesHTML}
    ...
    usageStatsContainer.innerHTML = statsHTML;
}

async function getUsageStatistics() {
    // This would normally fetch from storage/tracking system
    // For now, return mock data to demonstrate the UI
    return {
        totalUsage: 47,
        categoryStats: {
            'automotive': { usage: 15, templateCount: 8 },
            'fitness': { usage: 12, templateCount: 5 },
            'food': { usage: 10, templateCount: 6 },
            'home-services': { usage: 7, templateCount: 4 },
            'beauty': { usage: 3, templateCount: 2 }
        },
        topTemplates: [
            { id: '1', label: 'Car Service Special Offer', category: 'automotive', usageCount: 8 },
            { id: '2', label: 'Gym Membership Deal', category: 'fitness', usageCount: 6 },
            { id: '3', label: 'Restaurant Promotion', category: 'food', usageCount: 5 }
        ],
        recentActivity: [
            { templateLabel: 'Car Service Special Offer', category: 'automotive', timestamp: Date.now() - 3600000 },
            { templateLabel: 'Gym Membership Deal', category: 'fitness', timestamp: Date.now() - 7200000 },
            { templateLabel: 'Restaurant Promotion', category: 'food', timestamp: Date.now() - 10800000 }
        ]
    };
}
            ...
            <div class="suggestion-item group cursor-pointer hover:bg-gray-50 transition-all duration-200" 
                ...
                <div class="flex items-start space-x-3 p-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="suggestion-header flex items-start justify-between mb-2">
                            <div class="suggestion-text text-sm text-gray-900 leading-relaxed flex-1">
                                ${escapeHtml(suggestion.text)}
                            </div>
                            ${suggestion.category ? `<span class="suggestion-category bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full text-xs font-medium ml-2 flex-shrink-0">
                    ...
                    <div class="template-header flex justify-between items-start mb-2">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center space-x-2 mb-1">
                                ...
                                ${template.category ? `<span title="Category: ${getCategoryDisplayName(template.category)}" class="category-indicator bg-blue-100 text-blue-800 px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0">
                                    ${getCategoryDisplayName(template.category)}
                                </span>` : `<span title="No category assigned" class="category-indicator bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0">
                                    Uncategorized
                                </span>`}
                            </div>
                            <div class="template-stats flex items-center space-x-3 text-xs text-gray-500">
...
function getCategoryDisplayName(categoryId) {
    ...
    // Try to get from loaded categories
    const categoryFilter = document.getElementById('templateCategoryFilter');
    if (categoryFilter) {
        const option = categoryFilter.querySelector(`option[value="${categoryId}"]`);
        if (option) {
...
async function loadSettings() {
    try {
        ...
        // Load category preference and populate dropdown
        await loadCategories();
        
        // Set the selected category if one is saved
        const categorySelect = document.getElementById('categorySelect');
...
async function loadCategories() {
    try {
        // Initialize category manager if not already done
        if (!window.categoryManager) {
            const storageManager = new StorageManager();
            window.categoryManager = new CategoryManager(storageManager);
            await window.categoryManager.initialize();
        }
        
        const categories = await window.categoryManager.getAllCategories();
        
        // Populate main category selection dropdown
        const categorySelect = document.getElementById('categorySelect');
        if (categorySelect) {
        ...
        // Populate template category filter dropdown
        const templateCategoryFilter = document.getElementById('templateCategoryFilter');
        if (templateCategoryFilter) {
            // Clear existing options except "All Categories"
            templateCategoryFilter.innerHTML = '<option value="">All Categories</option>';
...
/**
 * Handle template category filter change
 */
function handleTemplateCategoryFilter(categoryId) {
    try {
        const searchTerm = document.getElementById('templateSearch')?.value.toLowerCase().trim() || '';
        const filterType = getCurrentFilter();
        
        // Apply the filter
        filterTemplates(searchTerm, filterType, categoryId);
        ...
        if (categoryId) {
            showSuccessMessage(`Filtering templates by category: ${categoryName}`);
        } else {
            showSuccessMessage('Showing all categories');
        }
        
        console.log('Template category filter applied:', categoryId);
    } catch (error) {
        console.error('Failed to apply category filter:', error);
        showErrorMessage('Failed to apply category filter');
    }
}

/**
 * Populate the template category dropdown in the template form
 */
async function populateTemplateCategoryDropdown() {
    try {
        // Initialize category manager if not already done
        if (!window.categoryManager) {
            const storageManager = new StorageManager();
        ...
        console.error('Failed to populate template category dropdown:', error);
        showErrorMessage('Failed to load categories');
    }
}
...
async function showTemplateModal() {
    ...
    await populateTemplateCategoryDropdown();
...
// Category modal management functions
...
function handleCategoryModalEscape(e) {
    if (e.key === 'Escape') {
        closeCategoryModal();
    }
}
...
function setupTemplateFormHandlers() {
    ...
    // Set up create category button handler
...
function validateTemplateForm(data) {
    ...
    if (!data.category) {
        errors.templateCategory = 'Please select a category for this template';
    }
...
function clearCategoryFormErrors() {
    // Remove error classes and ARIA attributes from category form
    const categoryModal = document.getElementById('createCategoryModal');
    categoryModal.querySelectorAll('.form-input.error').forEach(input => {
        input.classList.remove('error');
        input.setAttribute('aria-invalid', 'false');
        input.removeAttribute('aria-describedby');
    ...
    // Remove error messages from category form
    categoryModal.querySelectorAll('.form-error').forEach(error => {
        error.remove();
    });
}
...
async function handleCategoryFormSubmit(e) {
    ...
    if (!validation.isValid) {
        showCategoryFormErrors(validation.errors);
        return;
    }
    ...
    try {
        ...
        // Save category using category manager
        if (!window.categoryManager) {
            const storageManager = new StorageManager();
            window.categoryManager = new CategoryManager(storageManager);
            await window.categoryManager.initialize();
        ...
        await window.categoryManager.createCategory(categoryData);
...
function validateCategoryForm(data) {
    ...
    if (data.description && data.description.length > 200) {
        errors.categoryDescription = 'Description must be less than 200 characters';
    }
...
function setupImportExportHandlers() {
    ...
    // Export modal handlers - will be overridden by category pack functionality if available
...
function resetImportModal() {
    ...
    // Reset category pack specific checkboxes if they exist
    const allowPrebuiltOverwrite = document.getElementById('allowPrebuiltOverwrite');
    const updateExisting = document.getElementById('updateExisting');
    if (allowPrebuiltOverwrite) allowPrebuiltOverwrite.checked = false;
...
/**
 * Process the selected Category Pack file
 * @param {File} file - Selected file
 */
async function processAdPackFile(file) {
    // Use category pack functionality if available
    if (typeof processCategoryPackFile === 'function') {
        return await processCategoryPackFile(file);
    }
    
    // Fallback to original Ad Pack processing
    try {
        ...
        if (!validation.isValid) {
            showErrorMessage(`Invalid Category Pack format: ${validation.errors.join(', ')}`);
            return;
        }
...
async function startAdPackImport() {
    // Use category pack functionality if available
    if (typeof startCategoryPackImport === 'function') {
        return await startCategoryPackImport();
    }
    
    // Fallback to original Ad Pack import
    try {
...
/**
 * Handle export type change (original functionality)
 * @param {Event} event - Radio button change event
 */
function handleExportTypeChange(event) {
    // Check if category pack functionality is handling this
    if (typeof handleExportTypeChange !== 'undefined' && event.target.value === 'category') {
        return; // Let category pack functionality handle it
    }
...
/**
 * Handle original export functionality (fallback)
 */
async function handleOriginalExport() {
    return await startTemplateExport();
}
